...
fun <A, B> fold(ta: Tree<A>, l: (A) -> B, b: (B, B) -> B): B = when(ta) {
    is Leaf -> l(ta.value)
    is Branch -> b(ex.ex03.fold(ta.left, l, b), ex.ex03.fold(ta.right, l, b))
}

fun <A> sizeF(ta: Tree<A>): Int = ex.ex03.fold(ta, l = { 1 }) { b1, b2 -> 1 + b1 + b2 }

fun maximumF(ta: Tree<Int>): Int = ex.ex03.fold(ta, l = { it }) { b1, b2 -> max(b1, b2) }

fun <A> depthF(ta: Tree<A>): Int = ex.ex03.fold(ta, l = { 1 }) { b1, b2 -> max(b1 + 1, b2 + 1) }


fun <A, B> mapF(ta: Tree<A>, f: (A) -> B): Tree<B> =
    ex.ex03.fold<A, Tree<B>>(ta, l = { Leaf(f(it)) }) { b1, b2 -> Branch(b1, b2) }
